...
driver = get_driver()
json_path = os.path.abspath('./fox/data/paimon/')+"/mys_store/"
...
async def get_goods(game):
    ...
    for good in goods_list:
        ...
        good_info['time'] = time.strftime(
            '%Y-%m-%d %H:%M:%S', time.localtime(good['next_time']))
...
def save_exchange_info(user_id, state):
    ...
    path = Path() / 'fox' / 'data' / 'paimon' / 'mys_store' / f'{t}.json'
...
async def exchange_action(info):
    ...
    path = Path() / 'fox' / 'data' / 'paimon' / 'mys_store' / \
        f"{info['user_id']}-{info['商品']['id']}.json"
    path.unlink()
...
async def _():
    ...
    for exchange_data in path.iterdir():
        ...
        t = str(exchange_data).replace('fox\\data\\paimon\\mys_store\\', '').replace('fox/data/paimon/mys_store/', '').replace(
            '.json', '')
        scheduler.add_job(
            id=t,
            replace_existing=True,
...
def get_exchange_info(user_id):
    ...
    i = 1
    for exchange_data in path.iterdir():
        file_name = str(exchange_data).replace('fox\\data\\paimon\\mys_store\\', '').replace(
            'fox/data/paimon/mys_store/', '')
